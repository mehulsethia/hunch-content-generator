...
class TTSEngine:
    def __init__(self, tts_module, hunch_object: dict, path: str = "assets/temp/", max_length: int = DEFAULT_MAX_LENGTH):
    ...
    def run(self) -> Tuple[int, int]:
        ...
        if self.hunch_object.get("question"):
            self.call_tts("poll", self.hunch_object["question"])
        for idx, comment in enumerate(self.hunch_object.get("comments", [])):
            if self.length > self.max_length:
                break
            self.call_tts(f"comment_{idx}", comment["comment"])
