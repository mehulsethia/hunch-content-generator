...
def get_background_config(mode: str):
    ...
    try:
        print("Accessing settings.config:", settings.config)  # Debug print
        choice = str(settings.config.get("settings", {}).get("background", {}).get(f"background_{mode}", "default")).casefold()
    except Exception as e:
        print("Error in get_background_config:", e)
        choice = "default"  # Default value
...
def chop_background(background_config: Dict[str, Tuple], video_length: int, poll_id: str):
    ...
    if settings.config["settings"]["background"][f"background_audio_volume"] == 0:
        ...
        background_audio.write_audiofile(f"assets/temp/{poll_id}/background.mp3")
    ...
    try:
        ffmpeg_extract_subclip(
            ...
            start_time_video,
            end_time_video,
            targetname=f"assets/temp/{poll_id}/background.mp4",
        )
        ...
        with VideoFileClip(f"assets/backgrounds/video/{video_choice}") as video:
            ...
            new.write_videofile(f"assets/temp/{poll_id}/background.mp4")
